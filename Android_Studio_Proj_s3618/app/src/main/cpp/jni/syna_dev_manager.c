    ...
    switch (g_syna_dev) {
        case SYNA_RMI_DEV:
            ...
            retval = rmi_f54_read_report_frame(report_type, p_report_image,
                                               size_of_image, out_in_landscape);
            if (retval < 0) {
...
int syna_get_firmware_config(unsigned char* buf, int size_buf)
{
    ...
    switch (g_syna_dev) {
        ...
        case SYNA_TCM_DEV:
            ...
            if (retval < 0) {
#ifdef SAVE_ERR_MSG
                sprintf(err, "%s error: fail to get the static config\n", __func__);
                add_error_msg(err);
#endif
                return -EINVAL;
            }
    ...
    switch (g_syna_dev) {
        case SYNA_RMI_DEV:
            ...
            if (RAW_CMD_READ == type) {
                ...
                if (retval < 0) {
#ifdef SAVE_ERR_MSG
                    sprintf(err, "%s error: fail to read rmi reg 0x%x (len = %d)\n",
                            __func__, reg_addr, len);
                    add_error_msg(err);
#endif
                    return -EINVAL;
                ...
                if (retval < 0) {
#ifdef SAVE_ERR_MSG
                    sprintf(err, "%s error: fail to write rmi reg 0x%x (len = %d)\n",
                            __func__, reg_addr, len);
                    add_error_msg(err);
#endif
                    return -EINVAL;
        ...
        case SYNA_TCM_DEV:
            ...
            if (RAW_CMD_READ == type) {
                ...
                if (retval < 0) {
#ifdef SAVE_ERR_MSG
                    sprintf(err, "%s error: fail to read tcm packet (len = %d)\n",
                            __func__, size_in);
                    add_error_msg(err);
#endif
                    return -EINVAL;
            ...
            if (RAW_CMD_WRITE == type) {
                ...
                if (retval < 0) {
#ifdef SAVE_ERR_MSG
                    sprintf(err, "%s error: fail to write tcm packet, 0x%x 0x%x 0x%x\n",
                            __func__, packet[0], packet[1], packet[2]);
                    add_error_msg(err);
#endif
                    return -EINVAL;
 ...
 *         > 0, successfully collect the touched data
 */
int syna_query_touch_response_entry(int max_fingers_to_process)
{
    ...
    if (SYNA_RMI_DEV == g_syna_dev) {
        ...
        if (retval < 0) {
#ifdef SAVE_ERR_MSG
            sprintf(err, "%s error: fail to get the touch response\n", __func__);
            add_error_msg(err);
#endif
            return -EINVAL;
        }
...
int syna_get_pins_mapping(int rxes_offset, int rxes_len, int txes_offset, int txes_len)
{
    ...
    switch (g_syna_dev) {
        ...
        case SYNA_TCM_DEV:
            ...
            if (retval < 0) {
#ifdef SAVE_ERR_MSG
                sprintf(err, "%s error: fail to get the pins mapping\n", __func__);
                add_error_msg(err);
#endif
                return -EINVAL;
            }
