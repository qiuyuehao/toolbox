                                     ...
                                     int *limit_rt100, int size_of_limit_rt100)
{
    ...
    if (retval < 0) {
        printf_e("%s error: fail to do reset before rt100 testing\n", __func__);
#ifdef SAVE_ERR_MSG
        sprintf(err, "error: fail to do reset before rt100 testing\n");
        add_error_msg(err);
#endif
        retval = -EINVAL;
        goto exit;
    ...
    p_rt100_img1 = calloc((size_t)(tx * rx), sizeof(short));
    if (!p_rt100_img1) {
    ...
    p_rt100_img2 = calloc((size_t)(tx * rx), sizeof(short));
    if (!p_rt100_img2) {
    ...
    p_rt100_delta = calloc((size_t)(tx * rx), sizeof(short));
    if (!p_rt100_delta) {
