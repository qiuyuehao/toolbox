...
int tcm_get_touch_config()
{
    ...
    if ( retval < 0) {
        printf_e("%s error: fail to GET_TOUCH_REPORT_CONFIG\n", __func__);
#ifdef SAVE_ERR_MSG
    ...
    if (retval > TCM_TOUCH_CONFIG_SIZE) {
        printf_e("%s error: touch config size (%d) is out of TCM_TOUCH_CONFIG_SIZE (%d)\n",
                 __func__, retval, TCM_TOUCH_CONFIG_SIZE);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s: error: fail to GET_TOUCH_REPORT_CONFIG\n", __func__);
        add_error_msg(err);
#endif
        retval = -EIO;
        tcm_drop_package(retval);
        return retval;
    }
    ...
    while (idx < TCM_TOUCH_CONFIG_SIZE) {
        ...
        switch (code) {
            ...
            case TOUCH_GESTURE_ID:
                ...
                retval = tcm_get_data_from_report(entry, size, offset, bits, &data);
                if (retval < 0) {
                    printf_e("%s error: fail to get gesture id\n", __func__);
                    goto exit;
                }
                offset += bits;
                break;
            ...
            default:
                bits = g_tcm_handler.touch_config[idx++];
                offset += bits;
                printf_e("%s warning: unknown report code 0x%x\n",
                         __func__, code);
                break;
        }
    }
