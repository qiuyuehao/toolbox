                    ...
                    unsigned char* p_out, int size_out)
{
    ...
    do {
        ...
        if ( 0xA5 == header.marker) {
            ...
            printf_i("%s info: tcm header = 0x%x 0x%x 0x%x 0x%x\n",
...
/*
 * Function:  tcm_wait_for_mode_switch
 * --------------------
 * function to wait for the firmware mode change (app/bootloader)
 *
 * return: <0, error out
 *         otherwise, return the payload length
 */
int tcm_wait_for_mode_switch(void)
{
    int retval = 0;
    bool is_ready = false;
    int timeout_cnt = 0;
    ...
    do {
        ...
        if ( 0xA5 == header.marker) {
            /* device will return an identify report */
            if ( TCM_REPORT_IDENTIFY == header.code) {
                is_ready = true;
            }
        }
        timeout_cnt += 1;

    } while( (!is_ready) && (timeout_cnt < TCM_POLLING_TIMOUT) );
    ...
    retval = tcm_get_payload((unsigned char *)&identify_report, size_payload);
    if (retval < 0) {
        printf_e("%s error: fail to get the identify report (size = %d)\n",
                 __func__, size_payload);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s error: fail to get the identify report (size = %d)\n",
                __func__, size_payload);
        add_error_msg(err);
#endif
        return (-EIO);
    ...
    if (identify_report.mode == MODE_APPLICATION)
        retval = MODE_APPLICATION;
    else if (identify_report.mode == MODE_BOOTLOADER)
        retval = MODE_BOOTLOADER;
    else if (identify_report.mode == MODE_BOOTLOADER_TDDI)
        retval = MODE_BOOTLOADER_TDDI;
    else
        retval = 0xff;
...
/*
 * Function:  tcm_run_bootloader
 * --------------------
 * function to run bootloader firmware
 *
 * return: <0, fail to enter bootloader mode
 *         otherwise, succeed
 */
int tcm_run_bootloader(void)
{
    int retval = 0;
    unsigned char cmd_packet[3] = {CMD_RUN_BOOTLOADER_FIRMWARE, 0, 0};
#ifdef SAVE_ERR_MSG
    char err[MAX_ERR_STRING_LEN];
#endif
    ...
    if (retval < 0) {
        printf_e("%s error: fail to send command, CMD_RUN_BOOTLOADER_FIRMWARE\n", __func__);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s error: fail to send command, CMD_RUN_BOOTLOADER_FIRMWARE\n", __func__);
        add_error_msg(err);
#endif
        retval = -EINVAL;
        return retval;
    ...
    retval = tcm_wait_for_mode_switch();
    if ( retval < 0) {
        printf_e("%s error: fail to switch to bootloader firmware \n", __func__);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s error: fail to switch to bootloader firmware \n", __func__);
        add_error_msg(err);
#endif
        retval = -EIO;
        return retval;
    ...
    if ((retval != MODE_BOOTLOADER_TDDI) && (retval != MODE_BOOTLOADER)) {
        printf_e("%s error: fail to run bootloader firmware (retval = 0x%x)\n",
                 __func__, retval);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s error: fail to run bootloader firmware (retval = 0x%x)\n",
                __func__, retval);
        add_error_msg(err);
#endif
        retval = -EIO;
    ...
    printf_i("%s info: CMD_RUN_BOOTLOADER_FIRMWARE is completed\n", __func__);

    return retval;
}
/*
 * Function:  tcm_run_application
 * --------------------
 * function to run application firmware
 *
 * return: <0, fail to run application firmware
 *         otherwise, succeed
 */
int tcm_run_application(void)
{
    int retval = 0;
    unsigned char cmd_packet[3] = {CMD_RUN_APPLICATION_FIRMWARE, 0, 0};
#ifdef SAVE_ERR_MSG
    char err[MAX_ERR_STRING_LEN];
#endif
    ...
    retval = tcm_write_message(cmd_packet, sizeof(cmd_packet));
    if (retval < 0) {
        printf_e("%s error: fail to send command, CMD_RUN_APPLICATION_FIRMWARE\n", __func__);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s error: fail to send command, CMD_RUN_APPLICATION_FIRMWARE\n", __func__);
        add_error_msg(err);
#endif
        retval = -EINVAL;
        return retval;
    ...
    retval = tcm_wait_for_mode_switch();
    if ( retval < 0) {
        printf_e("%s error: fail to run application firmware \n", __func__);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s error: fail to run application firmware \n", __func__);
        add_error_msg(err);
#endif
        retval = -EIO;
        return retval;
    ...
    if (retval != MODE_APPLICATION) {
        printf_e("%s error: fail to run application firmware (retval = 0x%x)\n",
                 __func__, retval);
#ifdef SAVE_ERR_MSG
        sprintf(err, "%s error: fail to run application firmware (retval = 0x%x)\n",
                __func__, retval);
        add_error_msg(err);
#endif
        retval = -EIO;
    ...
    printf_i("%s info: CMD_RUN_APPLICATION_FIRMWARE is completed\n", __func__);
                         ...
                         int txguard_pins_offset, int txguard_pins_len)
{
    ...
    if ((cfg_data_len > offset_rx_pin + length_rx_pin) && (offset_rx_pin != 0)) {
        ...
        printf_i("%s info: get rx pins mapping, rx assigned = %2d\n",
    ...
    if (g_tcm_handler.guard_assigned > 0) {
        ...
        printf_i("%s info: get ground pins mapping, ground assigned = %2d (tx: %d, rx: %d)\n",
    ...
    if ((num_rx_guard > 0) &&
        ...
        for (i = 0; i < num_rx_guard; i++) {
            ...
            printf_i("%s info: rx_ground[%d] = %2d\n", __func__, i, g_tcm_handler.guard_pins[i]);
            idx += 2;
            j += 1;
        }
    }
